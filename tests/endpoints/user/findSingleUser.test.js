...
const { createUser, findUserById } = require('../../../src/api/v1/User')
...
describe("POST /users/id ", ()=>{
    it("should get a single user by admin", async()=>{
        ...
        // Get user by id
        const user = await request(app).get(`/users/${userCreate.id}`, findUserById).set('authorization', regUser.data?.access_token);
