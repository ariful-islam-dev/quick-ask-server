...
const { createUser, findUserById, deleteUser } = require('../../../src/api/v1/User')
...
describe("POST /users/id ", ()=>{
    it("should get a single user by admin", async()=>{
        ...
        const userCreate = await request(app).post("/users", createUser).send(createNewUser).set('authorization', regUser.data?.access_token);
        expect(userCreate.status).toBe(201);

        // Delete user by id
        const user = await request(app).delete(`/users/${userCreate.id}`, deleteUser).set('authorization', regUser.data?.access_token);
